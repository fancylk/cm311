...
fn initialize(app_dir: &str, custom_client_config: &str) {
    ...
    #[cfg(target_os = "android")]
    {
        ...
        // PATCH(surface-render): enable logcat logs in release too (tag "ffi")
...
pub mod server_side {
    ...
    #[no_mangle]
    pub unsafe extern "system" fn Java_ffi_FFI_setVideoSurface(
        env: JNIEnv,
        _class: JClass,
        surface: JObject,
    ) -> jboolean {
        // PATCH(surface-render): hand the Java Surface's native window to the
        // MediaCodec decoder so frames render directly to the display pipeline.
        #[link(name = "android")]
        extern "C" {
        ...
        let raw_surface = surface.as_raw() as *mut std::ffi::c_void;
        if raw_surface.is_null() {
            log::error!("setVideoSurface: null surface");
            return 0;
        }
        let nw = ANativeWindow_fromSurface(raw_env, raw_surface);
        if nw.is_null() {
            log::error!("ANativeWindow_fromSurface failed");
            return 0;
        }
        scrap::mediacodec::set_surface_ptr(nw as usize);
        log::info!("video surface set: {:#x}", nw as usize);
        1
    }
