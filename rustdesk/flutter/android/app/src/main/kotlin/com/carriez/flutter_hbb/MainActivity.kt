...
class MainActivity : FlutterActivity() {
    companion object {
        ...
        var videoSurfaceEntry: TextureRegistry.SurfaceTextureEntry? = null
                ...
                "createVideoSurface" -> {
                    try {
                        if (videoSurface == null) {
                            ...
                            videoSurfaceEntry = entry
                            videoSurface = st?.let { Surface(it) }
                        }
                        val surface = videoSurface
                        if (surface == null) {
                            result.success(mapOf("textureId" to -1L, "ok" to false))
                        } else {
                            val ok = FFI.setVideoSurface(surface)
                            val id = videoSurfaceEntry?.id() ?: -1L
                            Log.d(logTag, "createVideoSurface: id=$id ok=$ok")
                            result.success(mapOf("textureId" to id, "ok" to ok))
                        }
                    } catch (e: Exception) {
                        Log.e(logTag, "createVideoSurface failed: ${e.message}")
                        result.success(mapOf("textureId" to -1L, "ok" to false))
                    }
                }
                "init_service" -> {
