...
class FfiModel with ChangeNotifier {
  ...
  // PATCH(surface-render): Android hardware surface video path state
  int? _surfaceTextureId;
  bool _surfaceRender = false;
  bool get surfaceRender => _surfaceRender;
  int? get surfaceTextureId => _surfaceTextureId;
  /// textureId == null disables the surface view (keeps the last known id so
  /// it can be re-enabled when the negotiated codec becomes H264/H265 again).
  setSurfaceRender(int? textureId) {
    if (textureId != null) _surfaceTextureId = textureId;
    _surfaceRender = textureId != null && textureId >= 0;
    notifyListeners();
  }
  int? lastUserDisplay;
  ...
  updateEventListener(SessionID sessionId, String peerId) {
    // PATCH(csd-fix): re-enabled surface creation. The original disable was
    // based on "goke HEVC Surface mode broken at firmware level", but the NDK
    // harness proved the real blockers were (1) CSD never sent with flag=2
    // and (2) the hwcodec match arm shadowing the mediacodec arm in codec.rs.
    if (!isDesktop && !isWebDesktop) {
      Future.microtask(() async {
        try {
          final res = await platformFFI.invokeMethod('createVideoSurface');
          if (res is Map && res['ok'] == true) {
            final id = res['textureId'];
            setSurfaceRender(id is int && id >= 0 ? id : null);
          } else {
            setSurfaceRender(null);
          }
        } catch (e) {
          debugPrint('createVideoSurface failed: $e');
          setSurfaceRender(null);
        }
      });
    }
  ...
  updateQualityStatus(Map<String, dynamic> evt) {
    try {
      ...
      if (evt.containsKey('codec_format') &&
        ...
        // PATCH(surface-render): the surface view supports H264/H265/VP8
        // (MediaCodec+Surface). Fall back to the RGBA view for other codecs
        // (e.g. VP9, which has no surface-capable hardware decoder here).
        final ffiModel = parent.target?.ffiModel;
        if (ffiModel != null) {
          final cf = _data.codecFormat;
          final surfaceCapable =
              cf == 'H264' || cf == 'H265' || cf == 'VP8';
          if (surfaceCapable) {
            final id = ffiModel.surfaceTextureId;
            if (!ffiModel.surfaceRender && id != null && id >= 0) {
              ffiModel.setSurfaceRender(id);
            }
            // The private Goke decoder posts frames into this SurfaceTexture
            // without emitting an RGBA event. The codec report is the first
            // UI-visible indication that its surface path is active.
            if (ffiModel.surfaceRender) {
              parent.target?.onEvent2UIRgba();
            }
          } else if (ffiModel.surfaceRender) {
            ffiModel.setSurfaceRender(null);
          }
        }
      }
