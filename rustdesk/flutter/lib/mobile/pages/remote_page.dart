...
// PATCH(surface-render): continuously re-raster so the engine samples the
// newest frame from the SurfaceTexture (this embedding lacks a frame-available
// notification; without forced repaints the decoder stalls on full buffers).
class _SurfaceTexturePaint extends StatefulWidget {
  final int textureId;
  final double width;
  final double height;
  _SurfaceTexturePaint({Key? key, required this.textureId, required this.width, required this.height}) : super(key: key);

  @override
  State<StatefulWidget> createState() => _SurfaceTexturePaintState();
}

class _SurfaceTexturePaintState extends State<_SurfaceTexturePaint>
    with SingleTickerProviderStateMixin {
  late final AnimationController _ticker =
      AnimationController(vsync: this, duration: const Duration(days: 365))..repeat();
...
class ImagePaint extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // PATCH(surface-render): Android hardware surface path. The video is
    // rendered by MediaCodec directly into a SurfaceTexture; just place the
    // Texture to cover the remote display area. Pan/zoom transforms are not
    // applied in this mode (phase 1).
    if (ffiModel.surfaceRender && ffiModel.surfaceTextureId != null) {
      final displays = ffiModel.pi.displays;
      if (displays.isEmpty) {
        return const Offstage();
      }
      final idx = ffiModel.pi.currentDisplay.clamp(0, displays.length - 1);
      final w = displays[idx].width.toDouble();
      final h = displays[idx].height.toDouble();
      if (w <= 0 || h <= 0) {
        return const Offstage();
      }
      return _SurfaceTexturePaint(
        textureId: ffiModel.surfaceTextureId!,
        width: w,
        height: h,
      );
    }
