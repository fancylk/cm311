...
const OUTPUT: u32 = 0x47524631;
const MAX_RGBA: usize = 4096 * 2304 * 4;
...
fn render_loop(mut reader: UnixStream, window_ptr: usize) {
    ...
    loop {
        ...
        let (magic, width, height, len) = (words[0], words[1], words[2], words[3]);
        if magic != OUTPUT || width == 0 || height == 0 || width > 4096 || height > 2304
            || len as usize != width as usize * height as usize * 4 || len as usize > MAX_RGBA {
        ...
        unsafe {
            ...
            for row in 0..height as usize {
                ptr::copy_nonoverlapping(
                    ...
                    dst.add(row * buffer.stride as usize * 4),
                    width as usize * 4,
                );
            }
