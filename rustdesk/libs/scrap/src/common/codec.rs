...
pub struct Decoder {
    ...
    #[cfg(all(feature = "mediacodec", target_os = "android"))]
    h264_goke: Option<GokeDecoder>,
    #[cfg(all(feature = "mediacodec", target_os = "android"))]
    h265_goke: Option<GokeDecoder>,
    #[cfg(all(feature = "mediacodec", target_os = "android"))]
    vp8_media_codec: Option<MediaCodecDecoder>,
    format: CodecFormat,
    valid: bool,
    #[cfg(feature = "hwcodec")]
    i420: Vec<u8>,
...
impl Decoder {
    pub fn supported_decodings(
        ...
        // PATCH(surface-render): make sure MediaCodec probes are done before
        // advertising h264/h265 ability (cold-start race with login).
        #[cfg(all(target_os = "android", feature = "mediacodec"))]
        crate::mediacodec::ensure_mediacodec_probed();
        ...
        if enable_hwcodec_option() {
            ...
            // PATCH(csd-fix): goke VP8 hardware decode produces no live
            // output (EOS-flush-gated, same as HEVC buffer mode) and, worse,
            // attaching the session Surface to a VP8 decoder poisons later
            // HEVC Surface configure (ErrorUnknown forever; live test 13:11).
            // Report VP8 unsupported so the server picks H265 first and HEVC
            // becomes the Surface's first and only consumer.
    ...
    pub fn new(format: CodecFormat, _luid: Option<i64>) -> Decoder {
        ...
        #[cfg(feature = "mediacodec")]
        let (mut h264_media_codec, mut h265_media_codec, mut vp8_media_codec) = (None, None, None);
        #[cfg(all(feature = "mediacodec", target_os = "android"))]
        let (mut h264_goke, mut h265_goke) = (None, None);
        let mut valid = false;

        match format {
            CodecFormat::VP8 => {
                // PATCH(surface-render): prefer MediaCodec(goke VP8 hardware
                // decoder + Surface) and fall back to the software VP8 decoder.
                #[cfg(feature = "mediacodec")]
                if enable_hwcodec_option() {
                    vp8_media_codec = MediaCodecDecoder::new(format);
                    if vp8_media_codec.is_none() {
                        log::error!("create VP8 media codec decoder failed");
                    }
                }
                if vp8_media_codec.is_none() {
                ...
                valid = vp8_media_codec.is_some() || vp8.is_some();
            }
            CodecFormat::VP9 => {
                match VpxDecoder::new(VpxDecoderConfig {
            ...
            CodecFormat::H264 => {
                ...
                // PATCH(surface-render): on Android skip the hwcodec FFMPEG ram
                // decoder (software!) so MediaCodec+Surface handles H264/H265.
                ...
                if !valid && enable_hwcodec_option() {
                    h264_goke = GokeDecoder::new(format);
                    valid = h264_goke.is_some();
                }
                #[cfg(feature = "mediacodec")]
                if !valid && enable_hwcodec_option() {
            ...
            CodecFormat::H265 => {
                ...
                #[cfg(all(feature = "mediacodec", target_os = "android"))]
                if !valid && enable_hwcodec_option() {
                    h265_goke = GokeDecoder::new(format);
                    valid = h265_goke.is_some();
                }
                #[cfg(feature = "mediacodec")]
                if !valid && enable_hwcodec_option() {
    ...
    pub fn handle_video_frame(
        ...
        match frame {
            video_frame::Union::Vp8s(vp8s) => {
                // PATCH(surface-render): MediaCodec VP8 hardware decode first
                #[cfg(feature = "mediacodec")]
                if let Some(decoder) = &mut self.vp8_media_codec {
                    *_pixelbuffer = false;
                    return Decoder::handle_mediacodec_video_frame(decoder, vp8s, rgb);
                }
            ...
            // PATCH(csd-fix): on Android the mediacodec arm below must handle
            // H264s/H265s; with both features on, this arm shadows it and
            // always bails because h264_ram/h265_ram are deliberately None
            // (hwcodec-ram is ffmpeg software on Android).
            #[cfg(all(any(feature = "hwcodec", feature = "vram"), not(target_os = "android")))]
            ...
            video_frame::Union::H264s(h264s) => {
                *chroma = Some(Chroma::I420);
                ...
                if let Some(decoder) = &mut self.h264_goke {
                    return Decoder::handle_goke_video_frame(decoder, h264s);
                }
            ...
            video_frame::Union::H265s(h265s) => {
                *chroma = Some(Chroma::I420);
                #[cfg(target_os = "android")]
                if let Some(decoder) = &mut self.h265_goke {
                    return Decoder::handle_goke_video_frame(decoder, h265s);
                }
    ...
    #[cfg(all(feature = "mediacodec", target_os = "android"))]
    fn handle_goke_video_frame(
        decoder: &mut GokeDecoder,
        frames: &EncodedVideoFrames,
    ) -> ResultType<bool> {
        let mut accepted = false;
        for frame in frames.frames.iter() {
            accepted = decoder.decode(&frame.data)?;
        }
        Ok(accepted)
    }
