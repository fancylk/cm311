...
fn run(vs: VideoService) -> ResultType<()> {
    ...
    let (capture_width, capture_height) = capture_encoder_size(&c);
...
/// Cap the capture resolution fed to the encoder (macOS HiDPI displays expose
/// 2x framebuffers, e.g. 3840x2160 for a 1080p UI; the TV box decodes best at
/// <=1080p). Keeps the OS display settings untouched.
fn fit_capture_size(w: usize, h: usize, max_w: usize, max_h: usize) -> (usize, usize) {
    if w <= max_w && h <= max_h {
        return (w, h);
    }
    let mut dw = max_w;
...
fn capture_encoder_size(c: &CapturerInfo) -> (usize, usize) {
    if cfg!(target_os = "macos") {
        fit_capture_size(c.width, c.height, 1920, 1080)
    } else {
        (c.width, c.height)
    }
}
...
fn get_encoder_config(
    ...
    let (c_w, c_h) = capture_encoder_size(c);
