...
#[cfg(not(any(target_os = "ios")))]
/// A pixel buffer whose content is an area-averaged downscale of another
/// buffer, produced at creation time. Used to cap the resolution sent to the
/// encoder (e.g. capture 3840x2160 HiDPI -> encode 1920x1080) without touching
/// any OS display setting.
pub struct ScaledPixelBuffer {
    data: Vec<u8>,
    width: usize,
    height: usize,
    stride: usize,
...
impl ScaledPixelBuffer {
    pub fn downscale_from(src: &dyn TraitPixelBuffer, dst_w: usize, dst_h: usize) -> Self {
        let sw = src.width();
        let sh = src.height();
        let sstride = src.stride()[0];
        let bpp = src.pixfmt().bytes_per_pixel();
        let dstride = dst_w * bpp;
        let s = src.data();
        let mut data = vec![0u8; dstride * dst_h];
        for dy in 0..dst_h {
            let sy0 = dy * sh / dst_h;
            let sy1 = (((dy + 1) * sh / dst_h) as usize).max(sy0 + 1);
            let drow = &mut data[dy * dstride..(dy + 1) * dstride];
            let mut sx0 = 0usize;
            for dx in 0..dst_w {
                let sx1 = (((dx + 1) * sw / dst_w) as usize).max(sx0 + 1);
                let mut acc = [0u32; 4];
                let mut n = 0u32;
                for sy in sy0..sy1 {
                    let srow = &s[sy * sstride..];
                    for sx in sx0..sx1 {
                        let p = &srow[sx * bpp..sx * bpp + bpp];
                        for c in 0..bpp {
                            acc[c] += p[c] as u32;
                        }
                        n += 1;
                ...
                if bpp == 4 {
                    drow[dx * 4] = (acc[0] / n) as u8;
                    drow[dx * 4 + 1] = (acc[1] / n) as u8;
                    drow[dx * 4 + 2] = (acc[2] / n) as u8;
                    drow[dx * 4 + 3] = (acc[3] / n) as u8;
                } else {
                    for c in 0..bpp {
                        drow[dx * bpp + c] = (acc[c] / n) as u8;
                    }
                }
                sx0 = sx1;
            }
...
impl TraitPixelBuffer for ScaledPixelBuffer {
    fn data(&self) -> &[u8] {
        &self.data
    }
    fn width(&self) -> usize {
