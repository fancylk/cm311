...
static uint8_t *map_get(uint32_t phys){
    ...
    if(map_cache_n<MAPCACHE){map_cache[map_cache_n].phys=phys;map_cache[map_cache_n].ptr=p;map_cache_n++;}
        ...
        int slot=map_cache_next++%MAPCACHE;
        a.mmz_unmap(map_cache[slot].ptr);
        map_cache[slot].phys=phys;map_cache[slot].ptr=p;
    }
    return p;
}
...
static int nv21_from_frame(const uint32_t*f,Scratch*sc,uint32_t*out_w,uint32_t*out_h,const uint8_t**out_y,const uint8_t**out_vu) {
    ...
    if(ow!=w||oh!=h){
        /* Cache the address of each sampled pixel for all possible rows in a
           tile. The GK6323 row permutation depends on the horizontal lane. */
        uint32_t y_lut[16][MAX_OUTPUT_WIDTH];
        uint32_t vu_lut[8][MAX_OUTPUT_WIDTH/2];
        for(uint32_t col=0;col<ow;col++){
            uint32_t sx=(uint32_t)col*w/ow,tx=sx/64,k=sx%64;
            for(uint32_t ty=0;ty<16;ty++)
                y_lut[ty][col]=tx*1024+(ty^((tx&3u)<<2))*64+k;
        }
        for(uint32_t col=0;col<ow/2;col++){
            uint32_t sx=((uint32_t)(col*2)*w/ow)&~1u,tx=sx/64,k=sx%64;
            for(uint32_t ty=0;ty<8;ty++)
                vu_lut[ty][col]=tx*512+(ty^((tx&1u)<<2))*64+k;
        }
        for(uint32_t row=0;row<oh;row++){
            uint32_t sy=row*h/oh;
            uint32_t band=sy/16,ty=sy%16;
            uint8_t *dstrow=sc->yscale+row*ow;
            const uint8_t *srcrow=y+yoff+band*tiles*1024;
            const uint32_t *lut=y_lut[ty];
            for(uint32_t col=0;col<ow;col++)dstrow[col]=srcrow[lut[col]];
        }
        ...
        for(uint32_t row=0;row<oh/2;row++){
            ...
            const uint8_t *srcrow=vu+vuoff+band*tiles*512;
            const uint32_t *lut=vu_lut[ty];
            for(uint32_t col=0;col<uvcols;col++){
                const uint8_t *p=srcrow+lut[col];
                dstrow[col*2]=p[0];dstrow[col*2+1]=p[1];
            }
        }
    ...
    for(uint32_t row=0;row<h;row++){
        for(uint32_t tx=0;tx<(w+63)/64;tx++){
            ...
            uint32_t src=((row/16)*tiles+tx)*1024+((row%16)^((tx&3u)<<2))*64;
    ...
    for(uint32_t row=0;row<h/2;row++){
        for(uint32_t tx=0;tx<(w+63)/64;tx++){
            uint32_t n=w-tx*64;if(n>64)n=64;
            uint32_t src=((row/8)*tiles+tx)*512+((row%8)^((tx&1u)<<2))*64;
...
static void* drain_loop(void*arg){Drain*d=arg;Scratch sc={malloc(MAX_WIDTH*MAX_HEIGHT),malloc(MAX_WIDTH*MAX_HEIGHT/2),malloc(MAX_OUTPUT_WIDTH*MAX_OUTPUT_HEIGHT),malloc(MAX_OUTPUT_WIDTH*MAX_OUTPUT_HEIGHT/2),malloc(MAX_WIDTH*MAX_HEIGHT*4u)};
    ...
    while(!d->stop&&running){uint32_t f[1024]={0};
        ...
        /* 4K frames carry flag 0x20 and use an unhandled compressed layout.
           Close the bridge so RustDesk can fall back to its working VP9 path. */
        if(w>2880u||h>1800u||(f[16]&0x20u)){
