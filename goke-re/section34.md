...
- 真实远控测试在盒端连接后显示“已连接，等待画面传输”：Mac 官方 RustDesk 服务日志重复 `Failed to create capturer`（`CGDisplayStreamCreateWithDispatchQueue` 阶段）。独立 RustDeskCSD.app 先前也缺新 bundle ID 的录屏授权。当前不能把上述合成流结果冒充真实验收；用户正在核对 macOS“录屏与系统录音”中官方 RustDesk 的授权状态。失败原因尚未最终确认，外接屏当前也未出现在 `system_profiler SPDisplaysDataType`。
